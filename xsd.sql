...
<row>
  ...
  <xsd_file_pefix>HOUSE_TYPES</xsd_file_pefix>
</row>
<row>
...
select xsd_id,
       ...
       "cardinality",
       "length",
       "№" = 1 and "not null" = 'required' and transport_attribute = 'ID' "PK",
       xsda,
       table_name,
       t.xml_file_prefix
...
select loading_session_id,
       ...
       '    "' || column_name || '" '
       		...
       		|| case when "PK" or (column_name = 'Ключ' or (column_name ~ '^Код' and "№" = 1)) then ' primary key' else '' end
       		"ddl"
