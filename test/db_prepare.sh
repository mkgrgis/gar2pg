...
echo "Автоматический скрипт, расшифровка загруженных XSD (С ПОЛЕМ РЕГИОНА)
----------------------------------------------------------------------------"
cat "$ddf";

echo "______________________________________________________________________"

ddf='sql/ГАР_pgDDL0.sql';
echo "select 'CREATE SCHEMA \"' || set_config('ГАР.схема', :'схема_разворачивания_ГАР', false) ||'\";'; select ddl from xsd.table_ddl;" | psql -Atq --set=схема_разворачивания_ГАР="$3" --set=БД="$2" --set=атрибут_региона="Регион" > "$ddf";
# Прочитать сгенерированное
echo "Автоматический скрипт, расшифровка загруженных XSD (БЕЗ ПОЛЯ РЕГИОНА)
----------------------------------------------------------------------------"
cat "$ddf";
